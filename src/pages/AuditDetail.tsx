...
export default function AuditDetail() {
  ...
  const handleOpenHtmlReport = async () => {
    ...
    try {
      const { findings: rptFindings, envelope: rptEnvelope } = parseFindings(audit.ai_findings);

      const { data: rfiData } = await supabase
        .from("rfis")
        .select("title, category, priority, status, description")
        .eq("audit_id", audit.id);
      const rfis = rfiData || [];

      const opinionBadgeClass = (() => {
        const o = (audit.opinion || "").toLowerCase();
        if (o === "unqualified") return "background-color:#e0ffe0;color:#006400;";
        if (o === "qualified") return "background-color:#ffffcc;color:#808000;";
        if (o === "adverse") return "background-color:#ffe0e0;color:#800000;";
        return "background-color:#eee;color:#333;";
      })();

      const findingsRows = rptFindings.map(f => {
        const s = f.status.toLowerCase();
        const statusStyle = s === "pass" ? "color:#006400;" : s === "fail" ? "color:#c00;" : "color:#b8860b;";
        return `<tr>
          <td>${f.area}</td>
          <td style="${statusStyle}font-weight:bold;">${f.status}</td>
          <td>${f.detail || ""}</td>
          <td style="font-size:0.85em;color:#666;">${f.reference || ""}</td>
        </tr>`;
      }).join("");

      const rfiItems = rfis.map(r =>
        `<li style="margin-bottom:8px;">
          <strong>${r.title}</strong> (${r.category || "—"} · ${(r.priority || "medium").charAt(0).toUpperCase() + (r.priority || "medium").slice(1)})<br/>
          <span style="color:#444;">${r.description || ""}</span>
        </li>`
      ).join("");

      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/>
<title>${audit.fund_name} — Audit Report</title>
<style>
  body{font-family:-apple-system,Helvetica,Arial,sans-serif;color:#000;max-width:900px;margin:0 auto;padding:40px 24px;line-height:1.5;}
  h1{font-size:1.5em;margin-bottom:4px;}
  h2{border-bottom:1px solid #ccc;padding-bottom:4px;margin-top:32px;}
  table{width:100%;border-collapse:collapse;margin-top:12px;}
  th,td{border:1px solid #bbb;padding:6px 10px;text-align:left;vertical-align:top;font-size:0.9em;}
...
<p style="color:#666;margin-top:0;">${audit.financial_year ? `Financial Year ${audit.financial_year}` : ""}${audit.fund_abn ? ` · ABN ${audit.fund_abn}` : ""}</p>

<h2>Audit Opinion</h2>
<span class="badge" style="${opinionBadgeClass}">${(audit.opinion || "Pending").toUpperCase()}</span>
${rptEnvelope.opinion_reasoning ? `<p><strong>Reasoning:</strong> ${rptEnvelope.opinion_reasoning}</p>` : ""}
${rptEnvelope.summary ? `<p><strong>Summary:</strong> ${rptEnvelope.summary}</p>` : ""}

<h2>Compliance Findings</h2>
${rptFindings.length > 0 ? `<table><thead><tr><th>Area</th><th>Status</th><th>Detail</th><th>Reference</th></tr></thead><tbody>${findingsRows}</tbody></table>` : "<p>No findings.</p>"}

<h2>Requests for Information (${rfis.length})</h2>
${rfis.length > 0 ? `<ul>${rfiItems}</ul>` : "<p>No RFIs.</p>"}

<hr style="margin-top:40px;border:none;border-top:1px solid #ccc;"/>
<p style="font-size:0.8em;color:#999;">DRAFT — Generated by Auditron AI · ${new Date().toLocaleDateString("en-AU", { day: "numeric", month: "long", year: "numeric" })}</p>
</body></html>`;
      ...
      if (w) {
        ...
        toast({ title: "Pop-up blocked — please allow pop-ups for this site", variant: "destructive" });
      }
    } finally {
      setDownloading(null);
    }
